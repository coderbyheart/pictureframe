...
var PictureFrame = function () {
  function PictureFrame($, container, photos) {
    _classCallCheck(this, PictureFrame);
  ...
  _createClass(PictureFrame, [{
    ...
    value: function init() {
      ...
      this.elements = [];
      this.current = -1;
      this.photos.map(function (photo, key) {
        _this.photos[key].portrait = photo.width < photo.height;
        var type = _this.photos[key].portrait ? 'portrait' : 'landscape';
        _this.elements.push(_this.$('<img src="' + photo.url + '" alt="' + photo.title + '" width="' + photo.width + '" height="' + photo.height + '" class="photo ' + type + '">').appendTo(_this.container));
      });
      this.container.bind('touchstart', function (ev) {
        var xpos = ev.originalEvent.touches[0].pageX;
        if (xpos < _this.width / 4) {
          _this.gotoPrev();
          ev.preventDefault();
        }
        if (xpos > _this.width * 0.75) {
          _this.gotoNext();
          ev.preventDefault();
        }
      });
    ...
    value: function next() {
      ...
      this.resize();
      this.current++;
      var prev = this.elements[this.current - 1];
      if (prev) {
